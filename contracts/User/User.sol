...
contract User is IUser, Initializable, OwnableUpgradeable {
    ...
    modifier onlyAdmin() {
        _UserAccess().checkAccessModule(
            msg.sender,
            "User",
            uint(IUserAccess.AccessLevel.FOURTH)
        );
        _;
    }
     ...
     */
    function addUser(address account) external onlyAdmin returns (uint256) {
        if (users[account].last_updated != 0) revert AlreadyExist("User");
    ...
    function exist(address account) public view {
        if (users[account].id == 0) revert ObjectNotFound("User", 0);
    }
    ...
    function getUser(
        address account
    ) external view returns (IUser.User memory) {
        exist(account);
    ...
    function setPhone(
        ...
        if (!phone.isEncrypted()) revert ParamNotEncrypted("phone");
    ...
    function setEmail(
        ...
        if (!email.isEncrypted()) revert ParamNotEncrypted("email");
    ...
    function setTgId(
        address account,
        string calldata tg_id
    ) external onlyAdmin {
        if (!tg_id.isEncrypted()) revert ParamNotEncrypted("tg_id");
    ...
    function updateBaseData(
        ...
        uint access_level = _UserAccess().getModuleAccessLevel(
            "User",
            msg.sender
        );

        if (access_level <= uint(IUserAccess.AccessLevel.FIRST)) {
            revert AccessDeniedLevel(
                "User",
                uint8(IUserAccess.AccessLevel.FIRST)
            );
        }
    ...
    function addCar(address account, CarData calldata car_data) external {
        ...
        if (!car_data.brand.isEncrypted()) revert ParamNotEncrypted("brand");
        if (!car_data.model.isEncrypted()) revert ParamNotEncrypted("model");
    ...
    function updateCompanyInfo(
        address account,
        Company calldata company_data
    ) external {


        if (!company_data.name.isEncrypted()) revert ParamNotEncrypted("name");
        if (!company_data.description.isEncrypted()) revert ParamNotEncrypted("description");
        if (!company_data.inn.isEncrypted()) revert ParamNotEncrypted("inn");
        if (!company_data.kpp.isEncrypted()) revert ParamNotEncrypted("kpp");
        if (!company_data.ogrn.isEncrypted()) revert ParamNotEncrypted("ogrn");
        if (!company_data.bank_account.isEncrypted()) revert ParamNotEncrypted("bank_account");
        if (!company_data.bank_name.isEncrypted()) revert ParamNotEncrypted("bank_name");
        if (!company_data.bank_bik.isEncrypted()) revert ParamNotEncrypted("bank_bik");
        if (!company_data.bank_corr_account.isEncrypted()) revert ParamNotEncrypted("bank_corr_account");
        if (!company_data.bank_inn.isEncrypted()) revert ParamNotEncrypted("bank_inn");
        if (!company_data.bank_kpp_account.isEncrypted()) revert ParamNotEncrypted("bank_kpp_account");
