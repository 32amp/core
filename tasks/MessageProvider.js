...
async function encryptMessage(publicKeyHex, message) {
  ...
  const encrypted = await encrypt(publicKeyBytes, messageBytes); 
...
async function generateAESKey(keyLength = 256) {
  ...
  if (![128, 192, 256].includes(keyLength)) {
    throw new Error("Invalid key length. Supported lengths are 128, 192, and 256 bits.");
  }
  ...
  return new Promise((resolve, reject) => {
    crypto.randomBytes(keyByteLength, (err, buffer) => {
      if (err) {
        reject(err);
      } else {
...
messageProviderScope.task("generate-aes", "Generate AES256 key for encrypt message")
  .setAction(async () => {

    console.log("AES256 key:")
    console.log(await generateAESKey())
  });
...
messageProviderScope.task("listen-events", "Listen all events from contract MessageProvider")
  .setAction(async (taskArgs, hre) => {
    ...
    console.log(`Listen all events from contract ...`);

    messageProvider.on("*", (event) => {

      console.log(`Event ${event.fragment.name} call with args:`, event.args);
    });
...
messageProviderScope.task("register-provider", "Register a new provider")
  .setAction(async (taskArgs, hre) => {
    ...
    const smsGetway = {
      ...
      cost_per_message: answers.sms_enabled ? ethers.parseEther(answers.sms_cost_per_message) : 0,
      time_between_retry: answers.sms_enabled ? hre.ethers.toBigInt(answers.sms_time_between_retry) : 0
    };

    const emailGetway = {
      enable: answers.email_enabled,
      sender_name: answers.email_enabled ? answers.email_sender_name : '',
      cost_per_message: answers.email_enabled ? ethers.parseEther(answers.email_cost_per_message) : 0,
      time_between_retry: answers.email_enabled ? hre.ethers.toBigInt(answers.email_time_between_retry) : 0
    };
    ...
    try {
      ...
      const tx = await messageProvider.registerProvider(provider, {
        value: hre.ethers.parseEther(answers.deposit.toString())
      });
      await tx.wait();
      console.log("Provider registered successfully");
    } catch (error) {
      const decodedError = messageProvider.interface.parseError(error.data);
...
messageProviderScope.task("request-user-handshake-with-provider", "Request user handshake with provider")
  .setAction(async (taskArgs, hre) => {
    ...
    const provider = await messageProvider.getProvider(answers.provider)
    console.log("encryptMessage",await encryptMessage(provider.info.public_key, answers.aes_key))
    try {

      const tx = await messageProvider.requestUserHandshakeWithProvider(
        await encryptMessage(provider.info.public_key, answers.aes_key),
        answers.provider
      );
      let result = await tx.wait();
      console.log("Handshake request sent successfully", result);
    } catch (error) {
      const decodedError = messageProvider.interface.parseError(error.data);
      console.log("Decoded error:", decodedError);
    }
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    try {
      const tx = await messageProvider.requestSendSMS(
        answers.handshake,
        await encryptAESGCM(answers.recipient,answers.aes_key),
        await encryptAESGCM(answers.text,answers.aes_key),
        { value: hre.ethers.parseEther(answers.amount) }
      );
      await tx.wait();
...
messageProviderScope.task("request-send-email", "Request to send an Email")
  .setAction(async (taskArgs, hre) => {
    ...
    const tx = await messageProvider.requestSendEmail(
      answers.handshake,
      await encryptAESGCM(answers.recipient,answers.aes_key),
      await encryptAESGCM(answers.subject,answers.aes_key),
      await encryptAESGCM(answers.body,answers.aes_key),
      { value: hre.ethers.parseEther(answers.amount) }
    );
    await tx.wait();
